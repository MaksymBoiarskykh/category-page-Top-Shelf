...
function copyHtml() {
  return src(
    [
      `${settings.viewsDir.entry}/**/*.html`,
      `!${settings.viewsDir.entry}/inc/*.html`,
      `!${settings.viewsDir.entry}/includes/*.html`,
    ],
    { allowEmpty: true }
  )
    .pipe(cache("copyHtml"))
...
function minCss() {
  return src(
    `${settings.scssDir.output}/${settings.scssDir.mainFileName}.css`,
    { allowEmpty: true }
  )
    .pipe(
      plumber(function (error) {
...
function watching(cb) {
  watch(`${settings.scssDir.entry}/**/*.scss`, scss).on(
    "unlink",
    function (filePath) {
      delete cache.caches["scss"];
    }
  );
  watch(`${settings.jsDir.entry}/**/*.js`, copyScripts).on(
    "unlink",
    function (filePath) {
  ...
  watch(`${settings.viewsDir.entry}/inc/*.html`, copyHtmlInc).on(
    "change",
    function (filePath) {
      delete cache.caches["copyHtmlInc"];
    }
  );
  watch(`${settings.pugDir.entry}/**/_*.pug`, pug2html).on(
    "change",
    function (filePath) {
      delete cache.caches["pug2html"];
    }
  );
  watch(`${settings.pugDir.entry}/**/*.pug`, pug2html).on(
    "unlink",
    function (filePath) {
      delete cache.caches["pug2html"];
    }
  );
  watch(`${settings.assetsDir.entry}/**/*`, copyFiles).on(
    "unlink",
    function (filePath) {
...
if (settings.isPug) {
  ...
  exports.dist = series(
    ...
    cleanCache,
    cleanDist,
    parallel(scss, pug2html, copyScripts, copyFiles, imagesOptimisation)
  );
} else {
  console.log("HTML");
  exports.default = parallel(
